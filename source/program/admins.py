...
@bot.on_message(command(["تخطي", f"skip@{BOT_USERNAME}", "vskip"]) & other_filters)
@authorized_users_only
...
async def cbpause(ay, query: CallbackQuery):
    ...
    if not a.can_manage_voice_chats:
        return await query.answer("⋆ المشرف فقط لديه صلاحية الضغط علي الازرار", show_alert=True)
...
@bot.on_message(
    command(["مستوى الصوت", f"volume@{BOT_USERNAME}", "vol"]) & other_filters
)
@authorized_users_only
